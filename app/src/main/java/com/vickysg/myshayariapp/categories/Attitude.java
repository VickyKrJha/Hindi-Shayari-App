...
//        Query query = FirebaseDatabase.getInstance().getReference("attitude").orderByChild("DateTime").limitToFirst(getIntent().getIntExtra("size",0));
    ...
    private void next(){
        if (mInterstitialAd != null){
            mInterstitialAd.show(Attitude.this);
            position = (position +1) % quotes_list.size();
            quotes_txt.setText(quotes_list.get(position));
            count_txt.setText( (position+1) + "/" + quotes_list.size());
        }else{
